...
import { fetchGenerated150Reports } from "./generated-reports";
...
/**
 * Load the curated 150-report dataset (replaces all current reports). These
 * were generated by running 150 realistic resident scenarios through the live
 * Beacon pipeline (chat + formalize + photo captioning), so every report has a
 * genuine chat log, severity rating, and AI-written summary. Government
 * workflow (status, resolution times, declines) is simulated at load time.
 * See scripts/generate-150.mjs and public/data/govlink-reports-150.json.
 */
export async function loadGenerated150Reports(): Promise<void> {
  ensureLoaded();
  const reports = await fetchGenerated150Reports();
  setState({
    reports,
    hydrated: true,
  });
